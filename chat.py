...
5. How it compares to the reference essays"""
    ...
    try:
        while True:
            ...
            # Save the counselor's response to the chat history
            messages.append({"role": "assistant", "content": full_response})
            
            print("-" * 60)
            user_input = input("You (ask follow-ups or type 'exit' to quit): ")
            if user_input.strip().lower() in ['exit', 'quit']:
                print("Ending chat. Good luck with your essay!")
                break
                
            # Append the user's follow up query
